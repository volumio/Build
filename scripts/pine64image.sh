...
echo "Preparing for the ${MODEL} kernel/ platform files"
...
then
    ...
    cd platform-pine64
	if [ -f pine64-all.tar.xz ]; then
	   echo "Found a new tarball, unpacking..."
	   [ -d pine64-all ] || rm -r pine64-all
	   tar xfJ pine64-all.tar.xz
	   rm pine64-all.tar.xz
	fi
	cd ..
else
	echo "Clone ${MODEL} files from repo"
	wget https://github.com/volumio/platform-pine64/raw/master/pine64-all.tar.xz
...
cp platform-pine64/pine64-all/boot/boot.cmd /mnt/volumio/rootfs/boot

echo "Compiling boot script"
mkimage -C none -A arm -T script -d platform-pine64/pine64-all/boot/boot.cmd /mnt/volumio/rootfs/boot/boot.scr

echo "Copying kernel configuration file"
cp platform-pine64/pine64-all/boot/config* /mnt/volumio/rootfs/boot

echo "Copying kernel modules"
cp -pdR platform-pine64/pine64-all/lib/modules /mnt/volumio/rootfs/lib/

echo "Confguring ALSA with sane defaults"
cp platform-pine64/pine64-all/var/lib/alsa/* /mnt/volumio/rootfs/var/lib/alsa

echo "Copying firmware"
cp -R platform-pine64/pine64-all/firmware/* /mnt/volumio/rootfs/lib/firmware
sync

echo "Preparing to run chroot for more ${MODEL} configuration)"
...
#cleanup
rm /mnt/volumio/rootfs/root/init.sh /mnt/volumio/rootfs/pine64config.sh /mnt/volumio/rootfs/root/init
